...
{
    for (int i = 0; i < list->count; ++i) free(list->urls[i]);
    ...
    for (;;)
    {
        ...
        if (result == WAIT_OBJECT_0)  // Handle reconnect event
        {
            ...
            while ((error = tobii_device_reconnect(context->device)) != TOBII_ERROR_NO_ERROR)
            {
                ...
                if (result == WAIT_OBJECT_0) return 0;                   // exit thread
            }
...
static void log_func(void* log_context, tobii_log_level_t level, char const* text)
{
    ...
    if (level == TOBII_LOG_LEVEL_ERROR) fprintf(stderr, "Logged error: %s\n", text);
    LeaveCriticalSection(log_mutex);
}

static tobii_error_t device_connect(tobii_api_t* api, const char* url, const unsigned int retries,
                                    const unsigned int interval, tobii_device_t** device)
{
    fprintf(stdout, "Connecting to %s (trying %d times with %d milliseconds intervals).\n", url, retries, interval);
    ...
    {
        fprintf(stderr, "Failed connecting to %s", url);
        if ((error == TOBII_ERROR_CONNECTION_FAILED) || (error == TOBII_ERROR_FIRMWARE_UPGRADE_IN_PROGRESS))
        {
            fprintf(stderr, " (tried %d times with %d milliseconds intervals)", retries, interval);
        }
        fprintf(stderr, ".\n");
    }
...
int game_loop_sample_main(void)
{
    ...
    // Start subscribing to gaze point data, in this sample we supply a tobii_gaze_point_t variable to store latest
    // value.
    ...
    if (error != TOBII_ERROR_NO_ERROR) fprintf(stderr, "Failed to unsubscribe from gaze stream.\n");

    error = tobii_device_destroy(device);
    if (error != TOBII_ERROR_NO_ERROR) fprintf(stderr, "Failed to destroy device.\n");

    error = tobii_api_destroy(api);
    if (error != TOBII_ERROR_NO_ERROR) fprintf(stderr, "Failed to destroy API.\n");

...
#pragma warning(push)
#pragma warning( \
    disable : 4548 4265 4255 4668 4355 4625 4571 4626 5026 5027 4365)  // disable warnings triggered by STL code
#endif                                                                 // _MSC_VER
#include <atomic>
#include <iostream>
#include <mutex>
...
static auto list_devices(tobii_api_t* api)
{
    ...
    auto error = tobii_enumerate_local_device_urls(
        api,
...
static void log(void* log_context, tobii_log_level_t level, char const* text)
{
    ...
    if (level == TOBII_LOG_LEVEL_ERROR) std::cerr << "Logged error: " << text << std::endl;
}

static auto device_connect(tobii_api_t* api, std::string url, const unsigned int retries, const unsigned int interval,
                           tobii_device_t** device)
{
    std::cout << "Connecting to " << url << " (trying " << retries << " times with " << interval
              << " milliseconds intervals)." << std::endl;

    unsigned int retry = 0;
    auto error = TOBII_ERROR_NO_ERROR;
    do
    {
        error = tobii_device_create(api, url.c_str(), TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_FALSE, device);
...
{
    std::cout << "Reconnecting (trying " << retries << " times with " << interval << " milliseconds intervals)."
              << std::endl;
...
int background_thread_sample_main(void)
{
    ...
    std::thread thread([&exit_thread, device, interval, retries]() {
        while (!exit_thread)
        {
            ...
            if (error == TOBII_ERROR_TIMED_OUT)
                continue;  // If timed out, redo the wait for callbacks call
            ...
            {
                std::cerr << "tobii_device_process_callbacks failed: " << tobii_error_message(error) << "."
                          << std::endl;
                return;
            }
        }
    ...
    if (error != TOBII_ERROR_NO_ERROR) std::cerr << "Failed to unsubscribe from gaze stream." << std::endl;

    exit_thread = true;
    thread.join();

    error = tobii_device_destroy(device);
    if (error != TOBII_ERROR_NO_ERROR) std::cerr << "Failed to destroy device." << std::endl;

    error = tobii_api_destroy(api);
    if (error != TOBII_ERROR_NO_ERROR) std::cerr << "Failed to destroy API." << std::endl;

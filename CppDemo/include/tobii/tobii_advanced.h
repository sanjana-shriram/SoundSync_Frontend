...
typedef struct tobii_gaze_data_eye_t
{
    ...
    tobii_validity_t eye_position_validity;
    ...
    *timestamp_system_us* is generated using this value. The epoch is undefined, so these timestamps are only useful for calculating
    ...
    This is a struct containing the following data, related to the left eye:
        ...
        **TOBII_VALIDITY_INVALID** if *gaze_origin_mm_xyz* is not valid for this frame, **TOBII_VALIDITY_VALID** if it is.
        ...
        -   *eye_position_validity*
        **TOBII_VALIDITY_INVALID** if *eye_position_in_track_box_normalized_xyz* is not valid for this frame,
        **TOBII_VALIDITY_VALID** if it is.

        -   *eye_position_in_track_box_normalized_xyz*
        An array of three floats, for the x, y and z coordinate of the gaze origin point of the eye of the user, as
...
@code{.c}
    ...
    int main()
    {
        ...
        scanf( "%255s", url );
        error = tobii_device_create( api, url, TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_FALSE, &device );      // if not using a pro tracker use tobii_device_create_ex with Professional license
        assert( error == TOBII_ERROR_NO_ERROR );
...
/**
@fn TOBII_API tobii_error_t TOBII_CALL tobii_timesync( tobii_device_t* device, tobii_timesync_data_t* timesync );
@ingroup tobii_advanced
...
Establishes a synchronization point between two clocks: eye tracker clock and system host clock. Based on the *timesync*
data, we can compute the offset between the two clocks (see the example bellow).


### Syntax

    #include <tobii/tobii_advanced.h>
    tobii_timesync( tobii_device_t* device, tobii_timesync_data_t* timesync );
...
*timesync* is a valid pointer to a tobii_timesync_data_t instance to receive the information.

It constains the following fields:
...
*system_start_us* and *system_end_us* are used to calculate the round trip time (RTT) for the time sync operation.
The synchronization point is where *tracker_us* represents the same time as *system_start_us* + RTT / 2
...
-   **TOBII_ERROR_CALLBACK_IN_PROGRESS**
    ...
    tobii_device_process_callbacks(), tobii_calibration_retrieve(), tobii_enumerate_illumination_modes(),
    or tobii_license_key_retrieve().
    Calling tobii_timesync from within a callback function is not supported.
...
@code{.c}
    ...
    int main()
    {
        ...
        // if not using a pro tracker use tobii_device_create_ex with Professional license

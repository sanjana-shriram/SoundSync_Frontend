...
typedef struct tobii_responsive_gaze_point_t
{
    int64_t timestamp_us;
    tobii_validity_t validity;
    float position_xy[ 2 ];
} tobii_responsive_gaze_point_t;

typedef void (*tobii_responsive_gaze_point_callback_t)( tobii_responsive_gaze_point_t const* gaze_point, void* user_data );

TOBII_API tobii_error_t TOBII_CALL tobii_responsive_gaze_point_subscribe( tobii_device_t* device,
    tobii_responsive_gaze_point_callback_t callback, void* user_data );
TOBII_API tobii_error_t TOBII_CALL tobii_responsive_gaze_point_unsubscribe( tobii_device_t* device );
...
This function is not supported on all Tobii devices. In general we suggest using tobii_user_position_guide_subscribe() over
tobii_eye_position_normalized_subscribe(). Please contact Tobii support if you have questions.


### Return value

If the operation is successful, tobii_eye_position_normalized_subscribe returns **TOBII_ERROR_NO_ERROR**. Returns
**TOBII_ERROR_NOT_SUPPORTED** when not available. If the call fails, tobii_eye_position_normalized_subscribe returns an error code
...
| remote-system                | Cannot communicate with remote system                  |
|                              | (e.g. EyeChip or a peripheral device)                  |
...
The user position guide stream is used to help users position their eyes correctly in the track box frustum of a remote
eye tracker. It gives the current position of the left and right eye in normalized 3d-coordinates.
...
-   *user_position_guide*
    ...
    -   *left_position_normalized_xyz*
        ...
        Floating point array containing the normalized 3d-coordinate of the left eye in the range 0.0 to 1.0.
    ...
    -   *right_position_normalized_xyz*
        ...
        Floating point array containing the normalized 3d-coordinate of the right eye in the range 0.0 to 1.0.
...
/**
@fn TOBII_API tobii_error_t TOBII_CALL tobii_responsive_gaze_point_subscribe( tobii_device_t* device, tobii_responsive_gaze_point_callback_t callback, void* user_data );
@ingroup tobii_streams
...
Start listening for responsive gaze point data; the position on the screen that the user is currently looking at.


### Syntax

    #include <tobii/tobii_streams.h>
    tobii_error_t tobii_responsive_gaze_point_subscribe( tobii_device_t* device,
        tobii_responsive_gaze_point_callback_t callback, void* user_data );
...
This subscription is for receiving the point on the screen, in normalized (0 to 1) coordinates, that the user is
currently looking at. The data is not filtered for responsiveness.
...
If the operation is successful, tobii_responsive_gaze_point_subscribe returns **TOBII_ERROR_NO_ERROR**. If the call
fails, tobii_responsive_gaze_point_subscribe returns an error code specific to the device.

### See also

tobii_responsive_gaze_point_unsubscribe(), tobii_device_process_callbacks(), tobii_system_clock()
...
/**
@fn TOBII_API tobii_error_t TOBII_CALL tobii_responsive_gaze_point_unsubscribe( tobii_device_t* device );
@ingroup tobii_streams
...
Stops listening to gaze point stream that was subscribed to by a call to tobii_responsive_gaze_point_subscribe()


### Syntax

    #include <tobii/tobii_streams.h>
    tobii_error_t tobii_responsive_gaze_point_unsubscribe( tobii_device_t* device );
...
If the operation is successful, tobii_responsive_gaze_point_unsubscribe returns **TOBII_ERROR_NO_ERROR**. If the call
fails, tobii_responsive_gaze_point_unsubscribe returns an error code specific to the device.
...
See tobii_responsive_gaze_point_subscribe()

...
/**
@fn TOBII_API tobii_error_t TOBII_CALL tobii_device_create_ex( tobii_api_t* api, char const* url, tobii_field_of_use_t field_of_use, tobii_license_key_t const* license_keys, int license_count, tobii_license_validation_result_t* license_results, tobii_device_t** device );
...
*license_results* should be provided. It is an array for returning the results of the license validation for each license.
It is adviced the check *license_results* in any case. All the error's is related with licensing will only return by this
array.
...
@code{.c}
    ...
    int main()
    {
        ...
        tobii_device_t* device;
        error = tobii_device_create_ex( api, url, TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_FALSE, &license, 1, &validation_result, &device );
        free( license_key );
        assert( error == TOBII_ERROR_NO_ERROR );
...
-   **TOBII_FEATURE_GROUP_BLOCKED**
    ...
    The provided license key was invalid, or the application making the call has been blocklisted. No API functionality
    will be available.
...
which indicates that the specified license key was found to be invalid, or the current application has been blocklisted,

...
/**
@fn TOBII_API tobii_error_t TOBII_CALL tobii_device_create( tobii_api_t* api, char const* url, tobii_field_of_use_t field_of_use, tobii_device_t** device );
@ingroup tobii
...
When creating a connection to a Tobii eye tracker, you need to state whether or not your application is going to store
eye tracking data, or transfer eye tracking data. You do this by setting the parameter *field_of_use*,
to either TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_TRUE or TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_FALSE.

Examples:
-   If your application will store eye tracking data on the local machine you need to set the parameter
    to TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_TRUE.
-   If your application will transfer eye tracking data to a server, to another application, or a cloud system,
    you need to set the parameter to TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_TRUE.
-   If your application do not store eye tracking data, but only use it to determine immediate cause of action for
    your application, you need to set the parameter to TOBII_FIELD_OF_USE_STORE_OR_TRANSFER_FALSE.

Please note that eye tracking data refers to both raw eye tracking data, as well as eye tracking data in processed form.
This means that if you use the raw eye tracking data and aggregate it or process it into another form, it is still
eye tracking data.

The right to store or transfer eye tracking data is governed by Tobii’s software development licenses.
Please see https://developer.tobii.com/license-agreement/ for more information.

For more background information regarding Tobii’s requirements on storing and transferring eye tracking data,
and Tobii’s Eye Tracking Data Transparency policy, please see  https://transparency.tobii.com/.
...
purposes. (Deprecated name: TOBII_FIELD_OF_USE_INTERACTIVE)
...
(Deprecated name: TOBII_FIELD_OF_USE_ANALYTICAL)
...
This function is not supported on all Tobii devices, please contact Tobii support if you have questions.
